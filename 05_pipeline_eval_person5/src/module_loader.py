...
def get_ocr_process_document() -> Callable[[str], str]:
    ...
    broken_name = OCR_DIR / "trocr_engine (1).py"
    fixed_name = OCR_DIR / "trocr_engine.py"
    if broken_name.exists() and not fixed_name.exists():
        shutil.copyfile(broken_name, fixed_name)
