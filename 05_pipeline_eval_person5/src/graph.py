...
def grade_documents_node(state: GraphState, client: openai.OpenAI) -> GraphState:
    """
    Corrective RAG:
    - If a document was uploaded: compare chunks to DOCUMENT CONTENT.
    - If no document: compare chunks to the QUESTION.
    """
    ...
    # ✨ FIX: Choose the right reference
    if state.get("ocr_text"):
        # Document present → grade against document content
        grading_reference = state["ocr_text"][:500]
        log.info("Grading against DOCUMENT CONTENT")
    else:
        # No document → grade against question
        grading_reference = state["query_for_pipeline"]
        log.info("Grading against QUESTION")

    relevant, num_irrelevant = grade_chunks(
        client, grading_reference, chunks
    )

    if not relevant and chunks:
        log.warning("All chunks graded IRRELEVANT -> using all as fallback")
        relevant = chunks
